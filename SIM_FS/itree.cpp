	...
	Indirect *branch )
{
	int n = 0;
	int i;
	int parent = sim_new_block ( inode );

	branch[0].key = cpu_to_block ( parent );
	if ( parent ) for ( n = 1; n < num; n++ ) {
		struct buffer_head *bh;
		/* Allocate the next block */
		int nr = sim_new_block ( inode );
		if ( !nr )
			break;
	...
	for ( i = 1; i < n; i++ )
		bforget ( branch[i].bh );
	for ( i = 0; i < n; i++ )
		sim_free_block ( inode, block_to_cpu ( branch[i].key ) );
	return -ENOSPC;
}
	...
	int num )
{
	...
	if ( !verify_chain ( chain, where - 1 ) || *where->p )
	{
		...
		for ( i = 1; i < num; i++ )
			bforget ( where[i].bh );
		for ( i = 0; i < num; i++ )
			sim_free_block ( inode, block_to_cpu ( where[i].key ) );
		return -EAGAIN;
	}
...
{
	return ( block_t * ) sim_i ( inode )->i_data;
}
...
static int block_to_path ( struct inode * inode, long block, int offsets[DEPTH] )
{
	...
	if ( block < 0 ) {
		printk ( "sim_bmap: block<0" );
	}
	else if ( block >= ( sim_sb ( inode->i_sb )->s_max_size / BLOCK_SIZE ) ) {
		printk ( "sim_bmap: block>big" );
	}
	else if ( block < 7 ) {
		offsets[n++] = block;
	}
...
unsigned sim_blocks ( loff_t size )
{
	return nblocks ( size );
}
